...
class BlackBoxWrapper(gym.ObservationWrapper):
    ...
    def step(self, action: Union[np.ndarray, torch.Tensor]):
        ...
        if self.verbose >= 2:
            if self.backend == "torch":
                actions = torch.zeros(
                    size=(batch_size, trajectory_length) + self.env.action_space.shape,
                    device=self.device,
                )
                ...
                actions = np.zeros(
                    shape=(batch_size, trajectory_length) + self.env.action_space.shape
                )
        ...
        infos = dict()
        done = False  # TODO test without
        terminated, truncated = False, False

        if not traj_is_valid:
            obs, trajectory_return, terminated, truncated, infos = (
